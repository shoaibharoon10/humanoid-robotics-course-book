---
id: index
sidebar_position: 1
title: References
description: Bibliography and authoritative sources for Physical AI & Humanoid Robotics
keywords: [references, bibliography, citations, robotics, ROS 2, NVIDIA Isaac, VLA]
---

# References

This section provides authoritative sources and references for the concepts covered in this textbook. Citations follow APA 7th Edition format.

## Official Documentation

### ROS 2
- ROS 2 Documentation. (2024). *ROS 2 Jazzy Jalisco*. Open Robotics. https://docs.ros.org/en/jazzy/
- ROS 2 Design. (2024). *Design documentation*. Open Robotics. https://design.ros2.org/

### Simulation Platforms
- Gazebo Documentation. (2024). *Gazebo Harmonic*. Open Robotics. https://gazebosim.org/docs
- Unity Technologies. (2024). *Unity Robotics Hub*. https://github.com/Unity-Technologies/Unity-Robotics-Hub

### NVIDIA Isaac
- NVIDIA. (2024). *Isaac Sim Documentation*. https://docs.omniverse.nvidia.com/isaacsim/
- NVIDIA. (2024). *Isaac ROS*. https://nvidia-isaac-ros.github.io/

## Academic References

### Foundational Papers

Quigley, M., Conley, K., Gerkey, B., Faust, J., Foote, T., Leibs, J., Wheeler, R., & Ng, A. Y. (2009). ROS: An open-source Robot Operating System. *ICRA Workshop on Open Source Software*, 3(3.2), 5.

Koenig, N., & Howard, A. (2004). Design and use paradigms for Gazebo, an open-source multi-robot simulator. *IEEE/RSJ International Conference on Intelligent Robots and Systems*, 3, 2149-2154.

Macenski, S., Foote, T., Gerkey, B., Lalancette, C., & Woodall, W. (2022). Robot Operating System 2: Design, architecture, and uses in the wild. *Science Robotics*, 7(66), eabm6074.

### Vision-Language-Action

Brohan, A., et al. (2023). RT-2: Vision-Language-Action Models Transfer Web Knowledge to Robotic Control. *arXiv preprint arXiv:2307.15818*.

Driess, D., et al. (2023). PaLM-E: An Embodied Multimodal Language Model. *arXiv preprint arXiv:2303.03378*.

## Standards

- URDF Specification. http://wiki.ros.org/urdf
- SDF 1.9 Specification. http://sdformat.org/spec
- APA 7th Edition Citation Guidelines

---

*For additional resources and updates, visit the official documentation links above or use the Robotics Assistant chatbot.*
